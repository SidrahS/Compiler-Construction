...
import static semmentic.VariableTable.lookUp;
import static semmentic.VariableTable.Insert;

import static semmentic.FunctionTable.FunctionInsert;
import static semmentic.FunctionTable.FunctionLookUp;;

public class SemmenticMainClass
{
     private Token[] tokn;
     int i = 0;
     ...
     String Class_;                             //Current Class
     LinkedList ClassList = new LinkedList();   //ALL CLASSSES 
    ...
    public SemmenticMainClass()
    {
         System.out.println("IN SYMMENTIC_____________+++++++++++++++++++++++++______________________\n-------------------");
     ...
     private boolean ID_1()
    {
         ...
         if(tokn[i].CP.compareTo("As_Op")==0)
        {
             ...
             if(lookUp(Name, (int) ScopeList.getLast(), (String) ClassList.getLast())==null)
            {
                 Insert(Name, (int) ScopeList.getLast(),DT, AM, (String) ClassList.getLast());
            }
             if(Variable_Link2())
            {
                 return true;
    ...
    private boolean Method_Link3() {
             ...
             if (List_Param())
        {
             ...
             System.out.println("LIST PARM HAS ");
             for (int j = 0; j < ArgsList.size(); j++) {
                 System.out.println(j + " "+ArgsList.get(j));
            }
             if (tokn[i].CP.compareTo(")")==0)
            {
                if(FunctionLookUp(Name, (int) ScopeList.get(ScopeList.size()-2), (String) ClassList.getLast(),ArgsList)==null) /// FOR METHOD NAME SCOPE MUST BE -
                {
                     FunctionInsert(Name, (int) ScopeList.get(ScopeList.size()-2),DT,AM, (String) ClassList.getLast(),ArgsList); 
                     ArgsList.clear();
                }
                else{
                    error("main",tokn[i].lineNumber);
                    return false;
                }
     ...
     private boolean S_St_DT()
    {
         if (tokn[i].CP.compareTo("ID")==0)
        {
            if(lookUp(tokn[i].VP, (int) ScopeList.getLast(),Class_)==null)
            {
                Insert(tokn[i].VP, (int) ScopeList.getLast(),DT,null,Class_);            //null because variable is in main and has no AccessModifier
            }
            else
            {
                error(tokn[i].VP,tokn[i].lineNumber);
                return false;
            }
 ...
 private boolean List_Param()
{
     ...
     if(tokn[i].CP.compareTo("DT")==0)
    {
         ...
         if(tokn[i].CP.compareTo("ID")==0)
        {
             ...
             if(lookUp(Name, (int) ScopeList.getLast(), (String) ClassList.getLast())==null)
            {
                 Insert(Name, (int) ScopeList.getLast(), DT, null, (String) ClassList.getLast());
            }
             else
             {
                 error(Name,tokn[i].lineNumber);
                         return false;
             }
             i++;
...
return false ;
    ...
    private void error(String string,String line) {
        System.out.println("Variable "+string+" Alresy defined in line "+line);
    }
